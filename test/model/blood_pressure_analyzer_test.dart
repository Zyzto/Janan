...
void main() {
  ...
  test('firstDay and lastDay do not reorder the input list', () {
    final first = mockRecordPos(DateTime.fromMillisecondsSinceEpoch(100), 120);
    final middle = mockRecordPos(DateTime.fromMillisecondsSinceEpoch(20), 110);
    final last = mockRecordPos(DateTime.fromMillisecondsSinceEpoch(9000000), 130);
    final records = [first, middle, last];
    final analyzer = BloodPressureAnalyzer(records);

    expect(analyzer.firstDay, DateTime.fromMillisecondsSinceEpoch(20));
    expect(analyzer.lastDay, DateTime.fromMillisecondsSinceEpoch(9000000));
    expect(records, [first, middle, last]);
  });
