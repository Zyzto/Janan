...
class BleLaunchSyncCard extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                if (showResume)
                  IconButton(
                    ...
                    tooltip: localizations.resumeMeterSync,
                    onPressed: onResume,
                  ...
                  IconButton(
                    visualDensity: VisualDensity.compact,
                    tooltip: localizations.pauseMeterSync,
                    onPressed: onPause,
            ...
            if (_detail(localizations) case final String detail) ...[
  ...
  String _title(AppLocalizations localizations) {
    if (paused) return localizations.meterSyncPaused;
    final result = progress.result;
    if (result != null) {
      return switch (result.status) {
        BleLaunchSyncStatus.imported =>
        ...
        BleLaunchSyncStatus.notFound ||
        BleLaunchSyncStatus.cancelled => localizations.meterNotFound,
    ...
    return switch (progress.phase) {
      ...
      BleLaunchSyncPhase.scanning => hasName
          ? localizations.lookingForDevice(name)
          : localizations.scanningForDevices,
      BleLaunchSyncPhase.connecting => hasName
          ? localizations.foundMeterNamed(name)
          : localizations.connectingToMeter,
      BleLaunchSyncPhase.reading => localizations.readingStoredMeasurements,
      BleLaunchSyncPhase.importing => localizations.savingNewMeasurements,
      BleLaunchSyncPhase.done => localizations.syncingMeter,
    };
  }

  String _info(AppLocalizations localizations) {
    if (paused) return localizations.meterSyncPausedHint;
    final name = progress.deviceName?.trim();
    final hasName = name != null && name.isNotEmpty;
    final result = progress.result;
    if (result != null) {
      if (result.status == BleLaunchSyncStatus.imported
        ...
        if (result.duplicateCount > 0) {
          return localizations.skippedAlreadySaved(result.duplicateCount);
        }
        if (result.receivedCount > 0) {
          return localizations.newMeasurements(result.count);
        }
      }
      if (result.status == BleLaunchSyncStatus.notFound) {
        return localizations.keepMeterNearby;
      }
      return localizations.keepMeterNearby;
    }
    if (progress.lookingForMore) {
      return localizations.scanningForExtraDevices;
    }
    return switch (progress.phase) {
      BleLaunchSyncPhase.connecting => hasName
          ? localizations.connectingToDevice(name)
          : localizations.connectingToMeter,
      BleLaunchSyncPhase.reading => localizations.readingStoredMeasurements,
      BleLaunchSyncPhase.importing => localizations.savingNewMeasurements,
      BleLaunchSyncPhase.scanning => localizations.checkingForOneMinute,
      _ => localizations.keepMeterNearby,
    };
  }

  String? _detail(AppLocalizations localizations) {
    ...
    if (progress.phase == BleLaunchSyncPhase.reading
      ...
      return localizations.waitForMeterMeasurement;
    }
    return null;
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<int>(
      ...
      builder: (context, seconds, _) {
        ...
        return Text(
          localizations.timeLeft('$minutes:$rest'),
          style: Theme.of(context).textTheme.labelLarge,
        );
      },
    );
...
class _StepRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final steps = <(IconData, String)>[
      (Icons.bluetooth_searching, localizations.syncStepLook),
      (Icons.bluetooth_connected, localizations.syncStepConnect),
      (Icons.downloading, localizations.syncStepRead),
      (Icons.save_alt, localizations.syncStepSave),
    ];
    return Row(
      children: [
