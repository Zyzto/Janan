...
class BleGattSession with TypeLogger {
  ...
  Future<List<T>> collectNotifications<T>(
    ...
    if (onSubscribed != null) {
      try {
        ...
        logger.finer('onSubscribed failed: $error');
      }
    }
      ...
      keepAliveTimer = Timer.periodic(keepAliveInterval, (_) {
        unawaited(() async {
          try {
            ...
            logger.finer('keep-alive write failed: $error');
          }
        }());
      });
    }
    ...
    try {
      ...
      logger.finer('Failed to disable notifications: $error');
    }
