...
class YonkerProfile extends BleDeviceProfile with TypeLogger {
  ...
  Future<BleDeviceReadResult> read(BleGattSession session) async {
    logger.finest('YonkerProfile.read()');
    ...
    final data = await session.collectNotifications(
      ...
      settleFirst: false,
      decode: (value) {
        logger.fine('Received yonker notification: $value');
        return YonkerMeasurementData.decode(value)?.asBleData;
      },
      isComplete: (_, all) => all.isNotEmpty,
    );
    if (data.isEmpty) {
      logger.warning('Failed to decode yonker measurement for ${session.device}');
