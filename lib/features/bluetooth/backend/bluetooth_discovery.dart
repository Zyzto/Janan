...
abstract class BluetoothDeviceDiscovery<BackendDevice> with Loggable {
  /// Initialize base class for device discovery implementations.
  BluetoothDeviceDiscovery(this.manager) {
    logDebug('init device discovery: $this');
  }
  ...
  Future<void> start(ValueSetter<List<BluetoothDevice>> onDevices) async {
    if (_discovering) {
      logWarning('Already discovering, not starting discovery again');
      return;
    }
    ...
    _discoverSubscription = discoverStream.listen((newDevices) {
      logDebug('New devices discovered: $newDevices');
      assert(_discovering);
    ...
    logDebug('Starting discovery for devices');
    await backendStart();
  }
