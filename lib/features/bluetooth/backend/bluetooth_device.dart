...
abstract class BluetoothDevice with Loggable {
  ...
  Future<void> _waitForDisconnectingStateChange({ int timeout = 300000 }) async {
    ...
    Future<void> checkDeviceState() async {
      while (!futuresCompleted && _state == BluetoothDeviceState.disconnecting) {
        logDebug('Waiting because device is still disconnecting');
    ...
    if (timeout > 0) {
      futures.add(
        Future<void>.delayed(Duration(milliseconds: min(timeout, 300000))).then((_) {
          if (!futuresCompleted) {
            logDebug('connect: Wait for state change timed out after $timeout ms');
          }
        })
      );
    }
  ...
  Future<bool> connect({
    ...
    logDebug('connect: start connecting');
    ...
    _connectionListener = connectionStream.listen((BluetoothConnectionState state) {
      logDebug('connectionStream.listen[_state: $_state]: $state');
      ...
      switch (state) {
        case BluetoothConnectionState.connected:
          if (_state != BluetoothDeviceState.connecting) {
            ...
            logDebug('Ignoring state update because device was not connecting: $_state');
            return;
          }
        ...
        case BluetoothConnectionState.disconnected:
          if ([BluetoothDeviceState.connecting, BluetoothDeviceState.disconnected].any((s) => s == _state)) {
            ...
            logDebug('Ignoring state update because device was already disconnected: $_state');
            return;
          }
    ...
    try {
      ...
      logSevere('Failed to connect to device', error: e);
      if (!completer.isCompleted) completer.complete(false);
      _state = BluetoothDeviceState.disconnected;
    }

    return completer.future.then((res) {
      logDebug('connect: completer.resolved($res)');
      return res;
    });
  }
