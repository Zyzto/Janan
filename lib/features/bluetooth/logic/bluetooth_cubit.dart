...
class BluetoothCubit extends StateHolder<BluetoothState> with Loggable {
  ...
  BluetoothCubit({ required this.manager }):
    ...
    logDebug('lastKnownState: $_lastKnownState');
  }
  ...
  void _onAdapterStateChanged(BluetoothAdapterState state) async {
    if (state == BluetoothAdapterState.unauthorized) {
      ...
      if (success != true) {
        logWarning('Requesting bluetooth permission failed or not needed on this platform');
      }
    }

    _lastKnownState = state;
    logDebug('_onAdapterStateChanged(state: $state)');
