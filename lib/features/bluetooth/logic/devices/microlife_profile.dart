...
class MicrolifeProfile extends BleDeviceProfile with Loggable {
  ...
  Future<BleDeviceReadResult> read(BleGattSession session) async {
    logDebug('MicrolifeProfile.read()');
    ...
    final subscription = session.notifications(notifyCharacteristic).listen((value) {
      logDebug('Microlife notification: $value');
      buffer.addAll(value);
    ...
    try {
      ...
      try {
        ...
        logWarning('Microlife set time failed, continuing: $error');
      }
      ...
      try {
        ...
        logDebug('Microlife disconnect command failed: $error');
      }
      ...
      logWarning('Microlife communication timed out for ${session.device.uuid}');
      return const BleDeviceReadFailure('No data received');
    } catch (error) {
      logWarning('Microlife communication failed: $error');
      ...
      try {
        ...
        logDebug('Failed to disable microlife notifications: $error');
      }
    }
  }
}
