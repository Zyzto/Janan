...
class BleReadCubit extends StateHolder<BleReadState> with Loggable {
  ...
  }) : super(BleReadInProgress())
  {
    ...
    _timeoutTimer = Timer(const Duration(minutes: 2), () {
      if (state is BleReadInProgress) {
        logDebug('BleReadCubit timeout reached and still running');
        emit(BleReadFailure());
      } else {
        logDebug('BleReadCubit timeout reached with state: $state, ${state is BleReadInProgress}');
      }
    });
  }
  ...
  Future<void> close() async {
    logDebug('BleReadCubit close');
    await _subscription.cancel();
    _timeoutTimer.cancel();

    if (_device.isConnected) {
      try {
        logDebug('BleReadCubit close: Attempting disconnect from ${_device.advName}');
        await _device.disconnect();
        assert(_device.isDisconnected);
      } catch (e) {
        logSevere('unable to disconnect', error: [e, _device]);
      }
    }
  ...
  Future<void> _ensureConnection([int attemptCount = 0]) async {
    logDebug('BleReadCubit _ensureConnection');
    if (_ensureConnectionInProgress) return;
    _ensureConnectionInProgress = true;
    
    if (_device.isAutoConnectEnabled) {
      logDebug('BleReadCubit Waiting for auto connect...');
      _ensureConnectionInProgress = false;
      return;
    }
    
    if (_device.isDisconnected) {
      logDebug('BleReadCubit _ensureConnection: Attempting to connect with ${_device.advName}');
      try {
        await _device.connect();
      } on FlutterBlueUltraException catch (e) {
        logSevere('BleReadCubit _device.connect failed:', error: [_device, e]);
      }
      
      if (_device.isDisconnected) {
        logDebug('BleReadCubit _ensureConnection: Device not connected');
        _ensureConnectionInProgress = false;
        if (attemptCount >= 5) {
          emit(BleReadFailure());
          return;
        ...
        logDebug('BleReadCubit Connection successful');
      }
    }
  ...
  Future<void> _onConnectionStateChanged(BluetoothConnectionState state) async {
    logDebug('BleReadCubit _onConnectionStateChanged: $state');
    if (super.state is BleReadSuccess) return;
    if (state == BluetoothConnectionState.disconnected) {
      logDebug('BleReadCubit _onConnectionStateChanged disconnected: '
    ...
    try {
      ...
      logDebug('BleReadCubit allServices: $allServices');
    } catch (e) {
      logSevere('service discovery', error: [_device, e]);
      emit(BleReadFailure());
      return;
    }
    ...
    if (service == null) {
      logSevere('unsupported service', error: [_device, allServices]);
      emit(BleReadFailure());
      return;
    }
    ...
    logDebug('BleReadCubit allCharacteristics: $allCharacteristics');
    final BluetoothCharacteristic? characteristic = allCharacteristics
      .firstWhereOrNull((c) => c.uuid == characteristicUUID,);
    if (characteristic == null) {
      logSevere('no characteristic', error: [_device, allServices, allCharacteristics]);
      emit(BleReadFailure());
      return;
    }
    ...
    _indicationListener = characteristic
      .onValueReceived.listen((rawData) {
        logDebug('BleReadCubit data received: $rawData');
        final decodedData = BleMeasurementData.decode(rawData, 0);
        if (decodedData == null) {
          logSevere('BleReadCubit decoding failed', error: [ rawData ]);
          emit(BleReadFailure());
        } else {
          logDebug('BleReadCubit decoded: $decodedData');
    ...
    logDebug('BleReadCubit indicationsSet: $indicationsSet');
  }
}
