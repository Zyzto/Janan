...
/// Wrapper for FlutterBlueUltra in order to easily mock it
/// Wraps all calls for testing purposes
class FlutterBlueUltraMockable {
  LogLevel get logLevel => FlutterBlueUltra.logLevel;

  /// Checks whether the hardware supports Bluetooth
  Future<bool> get isSupported => FlutterBlueUltra.isSupported;

  /// The current adapter state
  BluetoothAdapterState get adapterStateNow => FlutterBlueUltra.adapterStateNow;

  /// Return the friendly Bluetooth name of the local Bluetooth adapter
  Future<String> get adapterName => FlutterBlueUltra.adapterName;

  /// returns whether we are scanning as a stream
  Stream<bool> get isScanning => FlutterBlueUltra.isScanning;

  /// are we scanning right now?
  bool get isScanningNow => FlutterBlueUltra.isScanningNow;

  /// the most recent scan results
  List<ScanResult> get lastScanResults => FlutterBlueUltra.lastScanResults;
  ...
  Stream<List<ScanResult>> get scanResults => FlutterBlueUltra.scanResults;

  /// This is the same as scanResults, except:
  /// - it *does not* re-emit previous results after scanning stops.
  Stream<List<ScanResult>> get onScanResults => FlutterBlueUltra.onScanResults;

  /// Get access to all device event streams
  BluetoothEvents get events => FlutterBlueUltra.events;

  /// Gets the current state of the Bluetooth module
  Stream<BluetoothAdapterState> get adapterState =>
      FlutterBlueUltra.adapterState;

  /// Retrieve a list of devices currently connected to your app
  List<BluetoothDevice> get connectedDevices =>
      FlutterBlueUltra.connectedDevices;
  ...
  Future<List<BluetoothDevice>> get bondedDevices =>
      FlutterBlueUltra.bondedDevices;
  ...
  }) => FlutterBlueUltra.setOptions(showPowerAlert: showPowerAlert);

  /// Turn on Bluetooth (Android only),
  Future<void> turnOn({int timeout = 60}) =>
      FlutterBlueUltra.turnOn(timeout: timeout);
  ...
  Future<void> stopScan() => FlutterBlueUltra.stopScan();
  ...
  Future<PhySupport> getPhySupport() => FlutterBlueUltra.getPhySupport();
}
