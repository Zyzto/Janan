...
class BleMeasurementData with Loggable {
  BleMeasurementData({
  ...
  static BleMeasurementData? decode(List<int> data, int offset) {
    ...
    if (data.length < 7) {
      Log.debug('BleMeasurementData decodeMeasurement: Not enough data, $data has less than 7 bytes.');
      return null;
    }
    ...
    if (data.length < (7
      ...
      Log.debug("BleMeasurementData decodeMeasurement: Flags don't match, $data has less bytes than expected.");
      return null;
    }
    ...
    if (systolic == null || diastolic == null || meanArterialPressure == null) {
      Log.debug('BleMeasurementData decodeMeasurement: Unable to decode required values sys, dia, and meanArterialPressure, $data.');
      return null;
    }
