...
class _FormSwitcherState extends State<FormSwitcher>
  ...
  void didUpdateWidget(covariant FormSwitcher oldWidget) {
    ...
    if (oldWidget.subForms.length != widget.subForms.length) {
      final nextLength = widget.subForms.length;
      final nextIndex = nextLength == 0
          ? 0
          : controller.index.clamp(0, nextLength - 1);
      controller.dispose();
...
class FormSwitcherController {
  ...
  void animateTo(int index) {
    ...
    _controller!.animateTo(index.clamp(0, _controller!.length - 1));
  }
